...
function LoginPreview({ theme, dark }: { theme: Theme; dark: boolean }) {
  const effectiveFont = FONT_OPTIONS.includes(theme.font_family ?? 'Inter') ? theme.font_family : theme.font_family;
  ...
  return (
    <div className="rounded-xl flex items-center justify-center min-h-[520px] p-6 transition-colors"
      ...
      <div className="w-72 rounded-xl p-7 shadow-xl space-y-4 transition-colors"
        ...
        {(theme.hydra_local_login ?? true) && (
          <div className="space-y-2">
            <div className="h-8 border" style={{ borderRadius: radius, borderColor: border, background: inputBg }} />
            <div className="h-8 border" style={{ borderRadius: radius, borderColor: border, background: inputBg }} />
            <div className="h-8 flex items-center justify-center text-white text-xs font-semibold"
              style={{ background: theme.primary_color ?? '#1a56db', borderRadius: radius }}>
              Sign in
            </div>
          </div>
        )}
...
export default function LoginTheme() {
  ...
  useEffect(() => {
    ...
    getProject(projectId).then(p => {
      if (p.login_theme) {
        ...
        // back-compat: providers without id get one assigned
        if (t.providers) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateProject(projectId, { login_theme: theme as Record<string, unknown> });
      setSaved(true);
      ...
      <div className="p-6 grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div>
          <Tabs defaultValue="visual">
            ...
            <TabsContent value="providers" className="mt-6 space-y-4">
              <div className="rounded-md border bg-amber-50 dark:bg-amber-950 border-amber-300 dark:border-amber-700 px-4 py-3 text-sm text-amber-800 dark:text-amber-300">
                Backend OAuth2 callback integration is not yet implemented. Provider buttons will appear on the login page but the flow is pending.
              </div>
        ...
        <div className="xl:sticky xl:top-6 space-y-3">
          ...
          <div className="rounded-xl border overflow-hidden">
            <LoginPreview theme={theme} dark={previewDark} />
          </div>

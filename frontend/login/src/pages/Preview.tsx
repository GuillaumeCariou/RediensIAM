 ...
 * the unauthenticated login page, and gstatic.com (or equivalent) would have to be allowed in
 * img-src for it. Add new providers the same way; do not reintroduce a remote <img> src.
 */
...
/**
 * Renders one of three cards from a base64 `cfg` query param: `mode === 'login'` and
 * `mode === 'register'` return early, and everything else falls through to the verify card at the
 * bottom — that fallthrough, not an explicit `'verify'` test, is what handles the third mode.
 *
 * `cfg` is attacker-supplyable (it is just a query param), so a malformed one must render
 * defaults rather than throw; that is what the empty catch below is for.
 */
